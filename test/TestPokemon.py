...
class TestCaseGameStruct(unittest.TestCase):
    ...
    def test_create_Pokemon(self):
        ...
        p = Pokemon("p", "FIRE", (100, 100, 100, 100, 100, 100), moves)

        test = [p.name, p.poke_type, p.cur_hp, p.hp, p.atk, p.des, p.spa, p.spd, p.spe, p.moves[0].name, p.moves[0].move_type, p.moves[0].base_pow, p.moves[1].name, p.moves[1].move_type, p.moves[1].base_pow]
        exp = ["p", "FIRE", 100, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50]
    ...
    def test_copy_Pokemon(self):
        ...
        test = [p_cp.name, p_cp.poke_type, p_cp.cur_hp, p_cp.hp, p_cp.atk, p_cp.des, p_cp.spa, p_cp.spd, p_cp.spe, p_cp.moves[0].name, p_cp.moves[0].move_type, p_cp.moves[0].base_pow, p_cp.moves[1].name, p_cp.moves[1].move_type, p_cp.moves[1].base_pow]
        exp = ["p", "FIRE", 100, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50]

        self.assertEqual(test, exp)

    @parameterized.expand([
        (["p", "FIRE", 100, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50], ["p", "FIRE", 100, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50], True),
        (["p", "GROUND", 100, 100, 100, 100, 100, 120, 100, "heavy_ground", "GROUND", 100, "light_grass", "GRASS", 50], ["p", "FIRE", 100, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50], False),
        (["p", "FIRE", 14, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50], ["p", "FIRE", 100, 100, 100, 100, 100, 100, 100, "heavy_fire", "FIRE", 100, "light_grass", "GRASS", 50], True)
    ])
    def test_cmp_Pokemon(self, p1_specs, p2_specs, exp):
        p1 = Pokemon(p1_specs[0], p1_specs[1], p1_specs[3:9], [Move(p1_specs[9], p1_specs[10], p1_specs[11]), Move(p1_specs[12], p1_specs[13], p1_specs[14])])
        p2 = Pokemon(p2_specs[0], p2_specs[1], p2_specs[3:9], [Move(p2_specs[9], p2_specs[10], p2_specs[11]), Move(p2_specs[12], p2_specs[13], p2_specs[14])])
    ...
    @parameterized.expand([
        (Pokemon("p1", "normal", [100] * 10, []), 0, True),
        (Pokemon("p1", "normal", [100] * 10, []), 75, True),
        (Pokemon("p1", "normal", [100] * 10, []), 100, False)
    ])
